...
from scipy.sparse import csc_matrix, csgraph, linalg
...
class Network:
    ...
    def loadNetwork(self, network_filename, binarize=False):
        ...
        if binarize:
            self.Adjacency.data = 1.0 * (self.Adjacency.data > 0)
    ...
    def standardize(self):
        ...
        standardized_adj_matrix = standardized_adj_matrix.maximum(
            standardized_adj_matrix.T
        )
    ...
    def netmf_embedding(self, T, skip_max=False):
        ...
        lap, deg_sqrt = sp.sparse.csgraph.laplacian(
            self.Adjacency, normed=True, return_diag=True
        )
        lam, W = np.linalg.eigh((sp.sparse.identity(n) - lap).todense())
        perm = np.argsort(-np.abs(lam))
        lam, W = lam[perm], W[:, perm]

        deg_inv_sqrt_diag = sp.sparse.spdiags(1.0 / deg_sqrt, 0, n, n)
        vol = self.Adjacency.sum()
        lam_trans = sp.sparse.spdiags(
            lam[1:] * (1 - lam[1:] ** T) / (1 - lam[1:]), 0, n - 1, n - 1
        )
        if skip_max:
            self.Embedding = np.log(
                1
                + vol
                / T
                * deg_inv_sqrt_diag
    ...
    def getNodesVolume(self):
        return (
            ...
            np.array(self.Adjacency.sum(axis=1)).flatten().sum(),
        )
...
class Optimizer:
    def __init__(
        ...
        self.device = device
        self.dtype = dtype
        self.pmi = torch.tensor(
            embedding, device=self.device, dtype=self.dtype, requires_grad=False
        )
        deg = np.array(adjacency.sum(axis=1)).flatten()
        self.vol = deg.sum()
        self.deg = torch.tensor(
            deg, device=self.device, dtype=self.dtype, requires_grad=False
        )
    ...
    def learnNetwork(self, max_iter=50, method="autoshift"):
        # FUNCTIONS
        def pmi_loss_10_elt_param(
            elts, n, logit_mode="raw", vol=0.0, skip_max=False, given_edges=False
        ):
            elts_tensor = torch.tensor(
                elts, device=self.device, dtype=self.dtype, requires_grad=True
            )
            ...
            if logit_mode == "individual":
                if not given_edges:
                    ...
                    adj_recon[np.triu_indices(n, 1)] = returnLearnedEdges(
                        n,
                        adj_recon,
                        elts_tensor,
                        given_edges,
                ...
                if not given_edges:
                    ...
                    adj_recon[np.triu_indices(n, 1)] = returnLearnedEdges(
                        n, adj_recon, elts_tensor, given_edges, activation=False
                    )
            elif logit_mode == "softmax":
                adj_recon[np.triu_indices(n, 1)] = torch.nn.functional.softmax(
                    elts_tensor, dim=0
                ) * (vol / 2)
                ...
                for i in range(10):
                    self.shift = (
                        self.shift
                        - (torch.sigmoid(elts_tensor + self.shift).sum() - (vol / 2))
                        / (
                            torch.sigmoid(elts_tensor + self.shift)
                            * (1.0 - torch.sigmoid(elts_tensor + self.shift))
                        ).sum()
                    )
                if not given_edges:
                    adj_recon[np.triu_indices(n, 1)] = torch.sigmoid(
                        elts_tensor + self.shift
                    )
                else:
                    adj_recon[np.triu_indices(n, 1)] = returnLearnedEdges(
                        n,
            ...
            p_recon = (1.0 / deg_recon)[:, np.newaxis] * adj_recon
            p_recon_2 = p_recon @ p_recon

            p_recon_5 = (p_recon_2 @ p_recon_2) @ p_recon
            p_geo_series_recon = (
                ((p_recon + p_recon_2) @ (torch.eye(n) + p_recon_2)) + p_recon_5
            ...
            loss_pmi = (pmi_recon_exact - self.pmi).pow(2).sum() / (self.pmi).pow(
                2
            ).sum()
        ...
        def callback_elt_param(x_i):
            ...
            if self.iter_num % 5 == 0:
                np.save(
                    "adj_recon/" + self.filename + "_" + self.rank + "_recon_elts.npy",
                    expit(self.elts + self.shift.detach().numpy()),
                )
        ...
        res = scipy.optimize.minimize(
            pmi_loss_10_elt_param,
            x0=self.elts,
            args=(self.n, "autoshift", self.vol, False),
            jac=True,
            method="L-BFGS-B",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Define filename, window, rank and limit"
    )
    parser.add_argument(
        "-f", "--filename", required=True, help="Relative path to dataset file"
    )
    parser.add_argument(
        "-i", "--it", required=False, default="1", help="Number of iteration"
    )
    parser.add_argument(
        "-w",
        "--window",
        required=False,
        default="10",
        help="Window for SGD (default is 10)",
    )
    parser.add_argument(
        "-r",
        "--rank",
        required=False,
        default="128",
        help="Rank of approximation for embedding (default is 128)",
    )
    parser.add_argument(
        "-l",
        "--limit",
        required=False,
        default="4",
        help="Limit the number of threads (default is 4)",
    )
    ...
    P = Optimizer(
        N.getAdjacency(), N.get_LR_embedding(), args.filename, args.it, args.rank
    )
    P.learnNetwork()
