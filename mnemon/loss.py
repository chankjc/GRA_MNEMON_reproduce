...
def graph_laplacian_regularization(adjacency_matrix, embedding_matrix):
    ...
    regularization_term = torch.trace(torch.dot(embedding_matrix.T, torch.dot(laplacian_matrix, embedding_matrix)))/2
    
    return regularization_term


def graph_sparsity_regularization(adjacency_matrix, alpha = 0.1, beta = 0.1):
    n = adjacency_matrix.shape[0]
    one = torch.ones(n).reshape(1,n)
    log_term = -1 * alpha * torch.dot(one, torch.log(torch.dot(adjacency_matrix, one.T)))
...
def graph_reconstruction_loss(adjacency_matrix_new, adjacency_matrix_old):
    ...
    temp = torch.multiply(adjacency_matrix_old, torch.log(adjacency_matrix_new)) + torch.multiply((1 - adjacency_matrix_old), (1 - torch.log(1 - adjacency_matrix_new)))
