...
def graph_laplacian_regularization(adjacency_matrix, embedding_matrix):
    ...
    regularization_term = torch.trace(torch.mm(embedding_matrix.T, torch.mm(laplacian_matrix, embedding_matrix)))/2
    
    return regularization_term


def graph_sparsity_regularization(adjacency_matrix, alpha = 0.1, beta = 0.1, device = "cuda:0"):
